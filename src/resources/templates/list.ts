 ...
 *
 * Returns the uniform `{ data, pagination }` envelope. Each `data` row
 * already carries the rendered `html` and a `previewImage` (plus
 * `title`/`category`/`brand`/`updatedAt`) — no per-template fetch is
 * needed. Supports exact `brand`/`category` filters, a lightweight
