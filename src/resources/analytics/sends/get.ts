...
export type GetSendInput = {
   ...
   * listed by `GET /v1/analytics/sends`. Cross-brand or unknown ids
   * surface as `404 SEND_NOT_FOUND` from the server.
   */
...
export function createGetSend(client: HttpClient) {
  ...
  async function getSend(
    ...
    const response = await client.request<GetSendResponse>({
      method: 'GET',
      path: `/v1/analytics/sends/${encodeURIComponent(input.sendId)}`,
      ...(options ? { options } : {}),
    })
